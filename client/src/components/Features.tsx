...
export default function Features() {
  ...
  const selectedFeatureIndex = appScreenshots.findIndex(
    (screenshot) => screenshot.title === selectedFeature
  );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setSelectedFeature(
        appScreenshots[(currentImageIndex + 1) % appScreenshots.length].title
      );
    }, 5000);
  ...
  const handleFeatureClick = (featureName: string) => {
    ...
    const index = appScreenshots.findIndex(
      (screenshot) => screenshot.title === featureName
    );
    if (index !== -1) {
  ...
  return (
    ...
    >
      <div className="container-custom">
        <div className="flex flex-col md:flex-row gap-6 items-top md:gap-10 max-w-6xl mx-auto mt-8 md:mt-12 px-0">
          <div className="md:w-1/2 lg:w-2/5">
            ...
            <div className="text-left mb-8  md:mb-10">
              ...
              <p className="text-gray-400 text-xs md:text-sm max-w-2xl px-0">
                Unlock a suite of smart tools that make content creation
                effortless<br></br>and brand engagement meaningful.
              </p>
            </div>
            ...
            <div className="space-y-4">
              {features.map((feature, index) => (
                ...
                >
                  <h4
                    className={`text-base md:text-lg font-semibold mb-2 md:mb-3 flex items-center gap-2 ${
                      selectedFeature === feature.title
                        ? "text-blue-highlight"
                        : "text-white"
                    }`}
